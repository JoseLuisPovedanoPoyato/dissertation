...
def get_cpu_data(service, request):
    ...
    consul_files = [f"consul/grouped_cpu_file_{u}_{request}_{service}" for u in USERS]
    final_labels = []
    ...
    if len(labels) > len(final_labels):
        final_labels = labels
    c_d, labels = get_data(consul_files)
    if len(labels) > len(final_labels):
        final_labels = labels

    return (k_d, i_d, l_d, c_d, final_labels)
 
# plot bars in stack manner
def gen_chart(k_d, i_d, l_d, c_d, labels):
    ...
    for u in range(len(USERS)):
        ...
        for label in range(len(labels)): #Could not change legend, so plot all Kubernetes first, then do the others
            ...
            plt.bar([l_c], c_d[u][label] if u < len(c_d) and label < len(c_d[u]) else 0, bottom=sum(c_d[u][:label]) if u < len(c_d) and label < len(c_d[u]) else 0, color=COLOURS[label])
...
for service in SERVICES:
    for req in REQUESTS:
        kube_dat,istio_dat,linkerd_dat,consul_dat,labels = get_cpu_data(service, req)
        gen_chart(kube_dat,istio_dat,linkerd_dat,consul_dat,labels)