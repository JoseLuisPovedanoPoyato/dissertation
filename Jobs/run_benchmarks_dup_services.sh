...
function delete_counter(){
    ...
    kubectl delete -f ${script_location}/../MicroCounterA/bare_counter_manifest.yml
    kubectl delete -f ${script_location}/../MicroCounterB/bare_counter_manifest.yml
    while true; do
        if kubectl get pods -l 'app=micro-counter-a' | grep -q 'micro-counter'; then
            echo "Some micro-counter pods are still running..."
            sleep 10
        else
            if kubectl get pods -l 'app=micro-counter-b' | grep -q 'micro-counter'; then
                echo "Some micro-counter pods are still running..."
                sleep 10
            else
...
function deploy_counter_bare() {
    ...
    deploy_counter ${script_location}/../MicroCounterA/bare_counter_manifest.yml
    deploy_counter ${script_location}/../MicroCounterB/bare_counter_manifest.yml
